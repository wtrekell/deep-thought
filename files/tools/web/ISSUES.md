...
| --- | --- | --- | --- | --- |
| R-01 | Critical | `image_extractor.py` | SSRF via `urlretrieve` with arbitrary URL schemes (`file://`, `ftp://`, cloud metadata) | Added URL scheme validation (http/https only), replaced `urlretrieve` with size-limited `urlopen`, filtered `data:` URIs |
...
| R-07 | High | `processor.py` | Unhandled exceptions in mode runners crash the entire crawl (violates per-URL error isolation requirement) | Added broad `except Exception` handler after specific handlers in all three mode runners |
...
| R-10 | High | `crawler.py` | Stealth mode UA mismatch (HTTP header vs JS `navigator.userAgent`) increased bot detection | Moved user-agent and viewport to `browser.new_context()` so both HTTP and JS values match |
| R-11 | High | `crawler.py` | Stealth viewport randomization was a no-op (`context.pages` empty when `_apply_stealth` called) | Resolved with R-10 — viewport set at context creation |

## Open — Medium

### M-01: Database connection never explicitly closed

- **File:** `cli.py` (line ~173)
- `initialize_database()` returns a connection that is never closed, even on error paths. The OS cleans up on exit, but dirty shutdowns can leave WAL/SHM files that must be replayed on next open.
- **Recommendation:** Wrap database usage in a `finally` block or context manager.

### M-02: `_PROJECT_ROOT` relies on file nesting depth

- **File:** `config.py` (line ~52)
...
- **File:** `processor.py` (lines ~540-562)
...
- **File:** `processor.py` (line ~98)
- `sqlite3.Connection` is accepted but never referenced. Misleading signature.
...
- **File:** `processor.py` (lines ~247-302)
...
- **File:** `db/schema.py` (lines ~186-191)
...
- **File:** `image_extractor.py` (lines ~95-96)
...
- **File:** `image_extractor.py` (lines ~62-68)
- Note: `data:` URIs are now skipped in `download_images` (R-01 fix), but they still enter the extracted URL list unnecessarily.
- **Recommendation:** Filter `data:` URIs in `extract_image_urls` as well.

### M-18: Playwright started in `__init__`, not in `__enter__`

- **File:** `crawler.py` (lines ~89-90)
...
### L-02: Requirements doc omits `title` from State Database column list

- **File:** `260322-requirements.md`
- Data Storage section lists columns without `title`, but Data Models section includes it. Code is correct.
- **Recommendation:** Update the requirements doc.

### L-03: Directory segments in output paths are not slugified

- **File:** `output.py` (lines ~73-74)
...
`load_config`, `_parse_crawl_config`, `validate_config` (10 validation checks) untested.
