...
1. **2026-03-13** — Build agents created for Python tool development (python-app-developer, sqlite-schema-expert, code-quality-integration)
...
1. **2026-03-21** — Reorganized non-code docs from `docs/` into `files/`: repo-rebuild log, future-enhancements, templates, and Todoist-specific docs (requirements, changelog, API model) now live under `files/`
1. **2026-03-21** — Added requirements drafts for 15+ future tools (PDF, Office, audio, video, image, text, data, email, markdown, web, Gmail, Reddit, research, Stack Exchange, YouTube, Krea, APNG, ElevenLabs) under `files/tools/00-requirements/`
1. **2026-03-21** — Moved Todoist YAML configuration to `src/config/todoist_configuration.yaml`
1. **2026-03-21** — Added `todoist create` subcommand: `create.py` business logic, `cmd_create` in `cli.py`, `upsert_task_with_labels` in `queries.py`, and supporting tests in `test_create.py`
1. **2026-03-21** — Added `DEEP_THOUGHT_DATA_DIR` env var support via `get_data_dir()` in `schema.py`; updated CLI help text and added `test_schema.py`
