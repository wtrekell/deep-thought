"""
schema.py — Database initialization and migration runner for the Stack Exchange Tool.
...
def _project_root() -> Path:
    ...
    for parent in current.parents:
        if (parent / "pyproject.toml").exists():
            return parent
    # Fallback: use the directory four levels up (db/ -> stackexchange/ -> deep_thought/ -> src/ -> root)
    return current.parents[4]


def get_data_dir() -> Path:
    """Return the base data directory for Stack Exchange tool storage.

    Checks DEEP_THOUGHT_DATA_DIR env var first; falls back to
    <project_root>/data/stackexchange.
    """
    env_override = os.environ.get("DEEP_THOUGHT_DATA_DIR")
    if env_override:
        return Path(env_override) / "stackexchange"
    return _project_root() / "data" / "stackexchange"


def get_database_path() -> Path:
    """Return the canonical path to the SQLite database file.

    Path: <data_dir>/stackexchange.db
    The parent directory is created if it does not already exist.
    """
    database_path = get_data_dir() / "stackexchange.db"
