"""Database initialization and migration runner for the web crawl tool.
...
- Track the applied migration version in a schema_version table
"""
...
def _project_root() -> Path:
    ...
    Determined by walking up from this file until pyproject.toml is found,
    ...
    for parent in current.parents:
        if (parent / "pyproject.toml").exists():
            return parent
    # Fallback: walk up from db/ -> web/ -> deep_thought/ -> src/ -> root
    return current.parents[5]


def get_data_dir() -> Path:
    """Return the base data directory for web tool storage.
...
def get_database_path() -> Path:
    ...
    Path: <data_dir>/web.db. The parent directory is created if it does not
    already exist.
...
def get_schema_version(conn: sqlite3.Connection) -> int:
    ...
    try:
        cursor = conn.execute(
            "SELECT value FROM web_schema_version WHERE key = ?;",
            (_SCHEMA_VERSION_KEY,),
        )
        row = cursor.fetchone()
...
def _set_schema_version(conn: sqlite3.Connection, version: int) -> None:
    """Persist the current schema version into web_schema_version.
    ...
    conn.execute(
        """
        INSERT INTO web_schema_version (key, value, updated_at)
        VALUES (?, ?, ?)
...
def run_migrations(conn: sqlite3.Connection, migrations_dir: Path) -> None:
    ...
    NNN is a zero-padded integer (e.g., 001_init_schema.sql). Files are sorted
    lexicographically, which keeps them in numeric order as long as the prefix
    width is consistent.

    Each migration is applied inside its own transaction. If a migration
    fails, the transaction is rolled back and the error is re-raised.
...
def initialize_database(db_path: Path | str | None = None) -> sqlite3.Connection:
    ...
    This is the primary entry point for setting up the database. Call once
    at application startup. The returned connection is ready for use.
