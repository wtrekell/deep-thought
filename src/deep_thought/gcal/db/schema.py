"""Database initialization and migration runner for the GCal Tool.
...
def _project_root() -> Path:
    ...
    for parent in current.parents:
        if (parent / "pyproject.toml").exists():
            return parent
    # Fallback: use the directory four levels up (db/ -> gcal/ -> deep_thought/ -> src/ -> root)
    return current.parents[4]


def get_data_dir() -> Path:
    """Return the base data directory for GCal tool storage.

    Checks DEEP_THOUGHT_DATA_DIR env var first; falls back to
    <project_root>/data/gcal.
    """
    env_override = os.environ.get("DEEP_THOUGHT_DATA_DIR")
    if env_override:
        return Path(env_override) / "gcal"
    return _project_root() / "data" / "gcal"


def get_database_path() -> Path:
    """Return the canonical path to the SQLite database file.

    Path: <data_dir>/gcal.db
    The parent directory is created if it does not already exist.
    """
    database_path = get_data_dir() / "gcal.db"
...
def run_migrations(conn: sqlite3.Connection, migrations_dir: Path) -> None:
    ...
    Each migration is applied inside its own transaction via
    ``conn.executescript()``, which issues an implicit COMMIT before running
    and handles the full SQL text as an atomic unit. If a migration fails,
    the error is re-raised, leaving the database in the last successfully
    applied state.
    ...
    for migration_file in migration_files:
        ...
        try:
            # executescript() issues an implicit COMMIT before execution, then
            # runs the entire script as a single atomic unit. This is simpler
            # and more correct than the manual split-on-semicolon approach.
