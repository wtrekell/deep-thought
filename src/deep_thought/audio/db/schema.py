"""
schema.py — Database initialization and migration runner for the Audio Tool.
...
def _project_root() -> Path:
    ...
    for parent in current.parents:
        if (parent / "pyproject.toml").exists():
            return parent
    # Fallback: use the directory four levels up (db/ -> audio/ -> deep_thought/ -> src/ -> root)
    return current.parents[4]


def get_data_dir() -> Path:
    """Return the base data directory for Audio tool storage.
...
def get_database_path() -> Path:
    ...
    Path: <data_dir>/audio.db
    The parent directory is created if it does not already exist.
    """
    database_path = get_data_dir() / "audio.db"
