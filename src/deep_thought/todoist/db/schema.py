"""
schema.py — Database initialization and migration runner for the Todoist Tool.
...
- Track the applied migration version in the sync_state table
"""
...
def _project_root() -> Path:
    ...
    for parent in current.parents:
        if (parent / "pyproject.toml").exists():
            return parent
    # Fallback: use the directory three levels up (db/ -> todoist/ -> deep_thought/ -> src/ -> root)
...
def get_schema_version(conn: sqlite3.Connection) -> int:
    """Read the current schema version from the sync_state table.
    ...
    try:
        ...
        # sync_state table does not exist yet — no migrations applied
        return 0


def _set_schema_version(conn: sqlite3.Connection, version: int) -> None:
    """Persist the current schema version into sync_state.
