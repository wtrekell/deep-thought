...
def _load_token_from_keychain(scopes: list[str]) -> Credentials | None:
    """Load OAuth credentials from the keychain for gdrive."""
    return _shared_load_oauth(_GDRIVE_SERVICE, scopes)


def _save_token_to_keychain(credentials: Credentials) -> None:
    """Persist OAuth credentials JSON to the keychain for gdrive."""
    _shared_save_oauth(_GDRIVE_SERVICE, credentials)


def _persist_credentials(credentials: Credentials, token_path: str, use_keychain: bool) -> None:
    """Save credentials to the appropriate storage backend for gdrive."""
    _shared_persist(_GDRIVE_SERVICE, credentials, token_path, use_keychain)
...
def get_credentials(credentials_path: str, token_path: str, scopes: list[str]) -> Credentials:
    ...
    Delegates to the shared ``get_oauth_credentials()`` function. See that
    function's docstring for full behavior details.
    ...
    """
    return get_oauth_credentials(_GDRIVE_SERVICE, credentials_path, token_path, scopes)
