...
- **Context flag:** `--context PATH` reads one or more prior research `.md` files and prepends their content to the user message sent to the API. Context is wrapped in structured XML tags so the model can distinguish prior research from the new query:
  ...
  ```xml
  <prior_research>
  <file path="data/research/export/260323-mlx-developments.md">
  ...file contents...
  </file>
  </prior_research>
...
research "What are the performance implications?" --context data/research/export/260323-mlx-developments.md
...
└── {YYMMDD}-{slug}.md             # Research results with YAML frontmatter
```
