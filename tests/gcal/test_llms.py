...
class TestStripFrontmatter:
    ...
    def test_no_frontmatter(self) -> None:
        ...
        assert strip_frontmatter(text) == "Just plain text."
...
class TestGenerateLlmsIndex:
    ...
    def test_generates_index(self, tmp_path: Path) -> None:
        ...
        file1 = _write_test_event_file(tmp_path, "260324-standup.md", "---\ntool: gcal\n---\n\nDaily standup notes.")
        file2 = _write_test_event_file(tmp_path, "260325-review.md", "---\ntool: gcal\n---\n\nWeekly review agenda.")
        result = generate_llms_index([file1, file2], "Personal")
        assert "# Personal — Event Index" in result
        assert "260324-standup.md: Daily standup notes." in result
        assert "260325-review.md: Weekly review agenda." in result
...
class TestGenerateLlmsFull:
    ...
    def test_generates_full_content(self, tmp_path: Path) -> None:
        ...
        assert "## 260324-standup.md" in result
...
class TestWriteLlmsFiles:
    ...
    def test_creates_llm_directory_and_files(self, tmp_path: Path) -> None:
        ...
        cal_dir.mkdir()
        file1 = _write_test_event_file(cal_dir, "260324-standup.md", "---\ntool: gcal\n---\n\nNotes.")
