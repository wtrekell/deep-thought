"""Tests for deep_thought.gmail._auth — OAuth token management via shared secrets module."""
...
def test_delegates_to_shared_oauth_with_gmail_service() -> None:
    """get_credentials should delegate to secrets.get_oauth_credentials with service='gmail'."""
    valid_creds = _make_valid_credentials()
    with patch("deep_thought.gmail._auth.get_oauth_credentials", return_value=valid_creds) as mock_get:
        result = get_credentials("/creds.json", "/token.json", _SCOPES)

    mock_get.assert_called_once_with("gmail", "/creds.json", "/token.json", _SCOPES)
...
def test_auto_migrates_file_to_keychain(tmp_path: Path) -> None:
    ...
    mock_save.assert_called_once_with("gmail", valid_creds)
